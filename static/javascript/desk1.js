...
Controller.initialize  =  function(eventObject){
 ...
 View.show_to_nav.addEventListener('click', Controller.show_to_nav);
    ...
    if (View.delete != null) {
        View.delete.addEventListener('click', function(event){
    localStorage.setItem("post_title",event.path[6].attributes[2].value);
    localStorage.setItem("post_email",event.path[6].attributes[3].value);
    localStorage.setItem("post_id",event.path[6].attributes[4].value);
    $.ajax({
        type: "POST",
        url: "/delete",
        data: JSON.stringify({
    ...
    Controller.deleteContent(event);
 });
    }
...
Controller.sittings = function(){
    alert("You have clicked the sittings button");
}
...
Controller.close     = function(event){
