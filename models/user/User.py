...
class Users(object):
    def __init__(self,firstname, lastname, email,password,image,phone, date=datetime.datetime.utcnow(),_id=None):
    ...
    def get_by_email(email):
        ...
        if data is not None:
            ...
            return False
    @staticmethod
    def  get_by_phone(phone, sender_email):
        phonedata = Database.find_one(UserConstants.COLLECTION, {"phone": phone})
        sender_phone_data = Database.find_one(UserConstants.COLLECTION, {"email":sender_email})
        if  phonedata is not  None and sender_phone_data is not None:
            if phonedata["phone"] == phone and sender_phone_data["phone"] != phonedata:
                return  True
            else:
                return   False
    ...
    @classmethod
    def registration(cls, firstname, lastname , email,password,img_user,phone):
        if not cls.get_by_email(email):
            utils.email_is_valid(email)
            new_user = cls(firstname,lastname,email,utils.hash_password(password), img_user,phone)
    ...
    def update_image(cls, email, image):
        Database.updates(UserConstants.COLLECTION,{"email":email },{"$set":{"image":"1"}})
        Database.updates(UserConstants.COLLECTION,{"email":email },{"$set":{"img":image}})
